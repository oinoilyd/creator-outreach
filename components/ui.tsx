...
/**
 * Brand-color tints per platform. When `colored` is on, the icon
 * uses these instead of inheriting `currentColor` from the parent
 * text — gives the platform pickers visual identity (red YouTube,
 * pink Instagram, blue LinkedIn) without changing the surrounding
 * text color.
 *
 * - YouTube: signature red.
 * - Instagram: pink (close enough to the gradient without needing
 *   a per-icon SVG gradient).
 * - TikTok: foreground (the brand mark is mostly black on white /
 *   white on black in their official assets).
 * - X / Twitter: foreground for the same reason — neutral mark.
 * - LinkedIn: signature blue.
 */
const PLATFORM_BRAND_COLOR: Record<PlatformId, string> = {
...
export function PlatformIcon({
  ...
  id: PlatformId
  className?: string
  /** When true, use the platform's brand color instead of inheriting
   *  `currentColor` from the parent. Defaults off for backwards
   *  compatibility with monochrome usages. */
  colored?: boolean
}) {
  const fullClass = colored
    ? `${className} ${PLATFORM_BRAND_COLOR[id]}`
    : className
  switch (id) {
    case 'youtube':
      return (
        <svg className={fullClass} viewBox="0 0 24 24" fill="currentColor">
